...
为了防止类似 bug 再出现，我后来给这个项目建了完整的 E2E 测试基建，**63 个用例**覆盖 **11 个 spec**（含 JWT、多端、二级认证等 1.1 场景），包括上面这个钩子的实际触发验证。
...
| --- | --- |
| 包大小（gzip） | **7.4 KB** |
| 单测覆盖率 | 96.03% Stmts / 91.97% Branch / 97.29% Funcs |
| E2E 覆盖率 | 96.08% Stmts / 91.39% Branch / 97.61% Funcs |
| 测试用例总数 | **274**（211 单测 + 63 E2E） |
