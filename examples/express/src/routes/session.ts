...
export function createSessionRouter() {
  ...
  router.post('/login-replace', asyncHandler(async (req, res) => {
    const { loginId } = req.body as { loginId: string };
    const token = await StpUtil.login(loginId);
    res.json({ token, loginId });
  }));

  router.post('/login-share', asyncHandler(async (req, res) => {
  ...
  router.post('/kickout', asyncHandler(async (req, res) => {
    const { loginId, device } = req.body as { loginId: string; device?: string };
    const ok = await StpUtil.kickout(loginId, device);
    res.json({ ok });
  }));

  router.post('/logout-by-login-id', asyncHandler(async (req, res) => {
    const { loginId } = req.body as { loginId: string };
    const ok = await StpUtil.logoutByLoginId(loginId);
    res.json({ ok });
  }));

  router.get('/online-count', asyncHandler(async (_req, res) => {
    const count = await StpUtil.getOnlineCount();
    res.json({ count });
  }));

  router.get('/online-ids', asyncHandler(async (_req, res) => {
