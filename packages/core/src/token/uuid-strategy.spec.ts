...
import { UuidStrategy } from './uuid-strategy.js';
...
describe('UuidStrategy', () => {
  ...
  it('createToken 支持 uuid / simple-uuid / random-32', () => {
    ...
    expect(strategy.createToken('1', makeConfig({ tokenStyle: 'simple-uuid' }))).toMatch(HEX_32_REGEX);
    expect(strategy.createToken('1', makeConfig({ tokenStyle: 'random-32' }))).toMatch(HEX_32_REGEX);
  });

  it('未知 tokenStyle 回退 random-32', () => {
    expect(strategy.createToken('1', makeConfig({ tokenStyle: 'unknown' as XltTokenConfig['tokenStyle'] }))).toMatch(
      HEX_32_REGEX,
    );
  });
});
