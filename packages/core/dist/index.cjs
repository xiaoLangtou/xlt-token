...
var UuidStrategy = class {
	...
	buildRaw(style) {
		switch (style) {
			case "uuid": return (0, node_crypto.randomUUID)();
			case "simple-uuid": return (0, node_crypto.randomUUID)().replace(/-/g, "");
			default: return (0, node_crypto.randomBytes)(16).toString("hex");
		}
	}
};
...
var StpLogic = class {
	...
	*/
	async login(loginId, options = {}) {
		if ((0, es_toolkit.isNull)(loginId) || (0, es_toolkit.isUndefined)(loginId) || loginId === "") throw new Error("invalid loginId");
	...
	*/
	...
	async refreshToken(token, timeout) {
		...
		if (this._isJwtMode()) try {
			...
			if (!payload.sub || !currentJti) return {
				ok: false,
				code: "TOKEN_INVALID"
			};
			if (await getStoreValue(this.store, this.keys.jwtBlacklistKey(currentJti))) return {
				ok: false,
				code: "TOKEN_INVALID"
			};
		} catch {
			...
			const loginId = await getStoreValue(this.store, this.keys.tokenKey(token));
			if (!loginId || loginId === NotLoginType.KICK_OUT || loginId === NotLoginType.BE_REPLACED) return {
				ok: false,
				code: "TOKEN_INVALID"
			};
		}
		...
		await setStoreValue(this.store, this.keys.tokenFamilyStateKey(nextToken), nextRaw, lifecycle.refresh.ttl);
		await setStoreValue(this.store, this.keys.tokenFamilyGenerationKey(nextToken, nextState.generation), nextToken, lifecycle.refresh.ttl);
		if (this._isJwtMode()) {
			const nextPayload = this.strategy.verifyToken(nextToken);
			await setStoreValue(this.store, this.keys.jwtBlacklistKey(currentJti), NotLoginType.KICK_OUT, resolvedTimeout);
			await setStoreValue(this.store, this.keys.sessionKey(state.loginId, state.device), nextPayload.jti, resolvedTimeout);
			if (this.config.activeTimeout > 0) await setStoreValue(this.store, this.keys.lastActiveKey(nextPayload.jti), String(now), resolvedTimeout);
		} else {
...
exports.StpLogic = StpLogic;
exports.StpPermLogic = StpPermLogic;
...
exports.XltMode = XltMode;
exports.XltSession = XltSession;
exports.XltTokenKeys = XltTokenKeys;
exports.createExpressContext = createExpressContext;
exports.createMockHttpContext = createMockHttpContext;
exports.createXltInstance = createXltInstance;
exports.createXltToken = createXltToken;
exports.finiteTtl = finiteTtl;
exports.getDefaultXltInstance = getDefaultXltInstance;
exports.keepTtl = keepTtl;
exports.matchPermission = matchPermission;
exports.normalizeDuration = normalizeDuration;
exports.normalizeTokenLifecycleConfig = normalizeTokenLifecycleConfig;
exports.normalizeXltTokenConfig = normalizeXltTokenConfig;
exports.persistentTtl = persistentTtl;
exports.setDefaultXltInstance = setDefaultXltInstance;
