...
import { StpPermLogic } from "../../src/auth/stp-perm-logic.js";
...
describe("StpPermLogic", () => {
  ...
  describe("checkPermission", () => {
    ...
    it("空 loginId 抛异常", async () => {
      await expect(logic.checkPermission("", ["user:add"], XltMode.AND)).rejects.toThrow(
  ...
  describe("checkRole", () => {
    ...
    it("AND 模式：缺少一个则抛 NotRoleException", async () => {
      await expect(logic.checkRole("admin", ["admin", "super"], XltMode.AND)).rejects.toThrow(
        NotRoleException,
      );
    });
    ...
    it("OR 模式：全部没有则抛 NotRoleException", async () => {
      await expect(logic.checkRole("viewer", ["admin", "super"], XltMode.OR)).rejects.toThrow(
        NotRoleException,
      );
    });

    it("空 loginId 抛异常", async () => {
      await expect(logic.checkRole("", ["admin"], XltMode.AND)).rejects.toThrow(NotRoleException);
    });
  });
