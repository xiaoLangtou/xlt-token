...
describe('Express adapter (e2e)', () => {
  ...
  describe('路由级 helper', () => {
    ...
    it('checkPermission 在真实 route chain 中执行权限校验', async () => {
      ...
      await request(app).get('/helper/order').set('authorization', adminToken).expect(200, { ok: true });

      const userToken = await login(app, '2002');
      const res = await request(app).get('/helper/order').set('authorization', userToken).expect(403);
      expect(res.body.code).toBe('NOT_PERMISSION');
    });

    it('checkRole 在真实 route chain 中执行角色校验', async () => {
      const adminToken = await login(app, '1001');
      await request(app).get('/helper/role-admin').set('authorization', adminToken).expect(200, { ok: true });

      const userToken = await login(app, '2002');
      const res = await request(app).get('/helper/role-admin').set('authorization', userToken).expect(403);
      expect(res.body.code).toBe('NOT_ROLE');
    });
