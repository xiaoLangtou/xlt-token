...
describe("相对时间 DurationInput (e2e)", () => {
  ...
  it("options.timeout 为 DurationInput 字符串时正确设置 TTL", async () => {
    ...
    const ttl = await store.getTtl(`authorization:login:token:${token}`);
