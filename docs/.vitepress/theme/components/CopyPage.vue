...
const isDoc = computed(
  () => frontmatter.value.layout !== 'home' && frontmatter.value.layout !== false && page.value.relativePath !== 'index.md',
)
...
async function downloadMarkdown() {
  ...
  window.setTimeout(() => {
    if (copied.value === 'download') copied.value = ''
  }, 1400)
}
</script>
  ...
  <div v-if="isDoc" class="xlt-copy-page">
    <div class="xlt-copy-page__actions" aria-label="Markdown page actions">
      ...
      <button class="xlt-copy-page__action" type="button" @click="downloadMarkdown">
        ...
        <span>{{ copied === 'download' ? 'Downloaded' : 'Download as Markdown' }}</span>
      </button>
    </div>
  </div>
</template>
